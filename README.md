...
> **好友机制（全自动）**：直连（局域网/蓝牙握手成功）的两台设备自动互为好友；
> 经中继被介绍、从未直连过的人会出现在会话列表（标记 ◇），直接输入消息即可——
> 会自动发送好友请求并排队，对方自动接受后消息自动送达。全程无需手动命令。
> 陌生设备的消息会被忽略，联系人也不会被泄露给陌生人；`/reject <ID>` 可拒绝并屏蔽某人。
...
- **好友机制（全自动）**：直连设备自动互为好友；经中继介绍的人出现在会话列表（◇），直接发消息即自动发送好友请求并排队，对方自动接受后送达。非好友消息忽略，联系人只在好友间共享，`/reject` 可屏蔽。
